				...
				if(firstSearch1==true){
					...
					if($.cookie("cookie.pumCd")!=undefined&&$.cookie("cookie.pumCd")!=''){
						...
						for(var i=0;i<res.list.length;i++){
							if(pumCd==res.list[i].itemCd){
								$("#itemCd").append("<option value='"+res.list[i].itemCd+"' selected>"+res.list[i].itemCd+"</option>");	
							}else{
								$("#itemCd").append("<option value='"+res.list[i].itemCd+"'>"+res.list[i].itemCd+"</option>");
							}
						}
						// 품목이 없을경우
			...
			success : function(res) {
				...
				if(firstSearch1==true){
					...
					if('${sessionScope.searchList}'!=null&&'${sessionScope.searchList}'!=''&&'${sessionScope.searchList.goodCd}'!=''){
						...
						goodCd = '${sessionScope.searchList.goodCd}';
						for(var i=0;i<res.list.length;i++){
							if(goodCd==res.list[i].kindNm){
								$("#itemCd2").append("<option value='"+res.list[i].kindNm+"' selected>"+res.list[i].kindNm+"</option>");	
							}else{
								$("#itemCd2").append("<option value='"+res.list[i].kindNm+"'>"+res.list[i].kindNm+"</option>");
							}
						}
						search();
						firstSearch1=false;
	...
	$.ajax({
		...
		success : function(res){
			if(res.list.length>0){
				$("#itemCd").html("");
				$("#itemCd").append("<option value='' selected> 전체 </option>");
				//getIntKindList(flowerCd, cmpCd, searchStrDate, searchEndDate, res);
				for(var i=0;i<res.list.length;i++){
 ...
 * 품종 리스트 검색하기
 */
function getIntKindList(flowerCd, cmpCd, searchStrDate, searchEndDate){
	$.ajax({
		type : 'post',
		...
		success : function(res){
			if(res.list.length>0){
				...
				$("#itemCd2").append("<option value='' selected> 전체 </option>");
				for(var i=0;i<res.list.length;i++){
					$("#itemCd2").append("<option value='"+res.list[i].pumJongCd+"'>"+res.list[i].pumJongName+"</option>");
					
				} search();
			}
		},
		error:function(res){
			alert("요청하신 페이지를 불러올 수 없습니다.");
			return;
		}
	});
}
