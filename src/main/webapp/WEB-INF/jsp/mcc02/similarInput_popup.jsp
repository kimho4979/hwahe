...
<%   
	if (request.getParameter("SEQ_NO")==null){
		...
		request.setAttribute("pageTitle", "월간장미수급예측보고서 수정");
	...
	form: {
		submit: function(){
			...
			if(confirm("등록하시겠습니까?")==true){
				...
				$.ajax({
					type : "POST",
					url : "/mntr/mcc02/insertSimilarInput.json",
					data : formData, 
					contentType: false,
					processData: false,
		...
		submit2: function(){				
			...
			if(confirm("수정하시겠습니까?")==true){
				...
				$.ajax({
					type : "POST",
					url : "/mntr/mcc02/updateSugeupInfoInput.json",
					data : formData,
					contentType: false,
					processData: false,
					success : function(res){
	...
	<article>
		<div>
			...
			<form name="send-form" id="send-form" enctype="multipart/form-data">
				...
				<input type=hidden id="GUBN" name="GUBN" value="4"/>
				...
				<div class="hw_infouse_01"><input type="text" class="AXInput width94p" id="TITLE" name="TITLE" value="${TITLE}"/></div>
				<div class="hw_clear"></div>
				
				<h4 class="ma_title">작성자</h4> <p class="hw_red"></p>
				<div class="hw_clear"></div>
				<div class="hw_infouse_01"><input type="text" id="WRITE_NAME" class="AXInput" name="WRITE_NAME" value="${WRITE_NAME}"/></div>
				<div class="hw_clear"></div>
				...
				<div class="hw_infouse_01">
					<span id="fileEl" >
						<a href="/mcc02/mcc02FileDown.json?seq=${SEQ_NO}">${ORIGINAL_FILE_NAME}</a>
					</span>
