...
$(document).ready(function(){	
	$(".main_title").html($("#stdYear").prop("value") + "년 전국 절화 농가 현황");
	fn_mapInit();
});
	...
	$.ajax({
	    ...
	    success : function(res){        
	    	...
	    	for(var i=0; i<resultList.length-1; i++){
	    		if(resultList[i].gubun == "1" ){
	    			if(max < resultList[i].prdArea){
		    			max = resultList[i].prdArea;
		    		}	
	    		}
	    	}
	 ...
	 layoutStr = '<rMateMapChart>'
			  ...
			  +'</series>'
			  +'</MapChart>'
			  +'<DataRangeLegend width="200" height="35" minimum="0" maximum="'+max+'" dataRangeHeight="20" fontSize="10" arrowOffset="0" interval="20" showArrow="true" tickGap="5" stroke="#000000" arrowColor="#ffffff" arrowStroke="#000000" colors="[#dcdfe8,#93D6F1,#4a88b7,#596283,#5b5e67]"/>'
			  +'</rMateMapChart>';
...
var layoutStr10 = 
	...
	+'<Options>'
		+'<Caption text=""/>'
	+'</Options>'
	+'<CurrencyFormatter id="cft" currencySymbol="%" precision="1" alignSymbol="right"/>'
	/**
	 ...
	 */
	+'<Gauge height="380" formatter="{cft}" innerRatio="0.85" labelJsFunction="valueLabelFunc" foregroundColors="[#f6a44c]" backgroundColors="[#e8d7c9]" minimum="0" maximum="300" startAngle="-90" minimumAngle="0" maximumAngle="180" color="#333333" fontSize="53" verticalOriginRatio="0.7" labelYOffset="-50" >'
		+'<backgroundElements>'
			+'<CanvasElement>'
		        +'<Label fontSize="13" height="17" color="#888888" horizontalCenter="0" verticalCenter="65" text=""/>'
		   +'</CanvasElement>'
		+'</backgroundElements>'
	+'</Gauge>'
+'</rMateChart>';
...
function fn_chart(stdYear, sidoCode, sigunCode, prdArea){
	...
	$.ajax({
	    ...
	    dataType : 'json',   	           
	    success : function(res){        
	    	console.log(res.chulInfoChartVO);
	    	if(res.chulInfoChartVO.length > 0){
		    	...
		    	chartData10 = [res.chulInfoChartVO[0].b1r1c51];
		    	...
		    	if(document.getElementById("chart10") == null){
		    		rMateChartH5.create("chart10", "chartHolder10", chartVars10, "100%", "100%");
		    	}else{
		    		document.getElementById("chart10").setData([res.chulInfoChartVO[0].b1r1c51]);
		    	}
	...
	<div class="wrap">
		...
		<div class="header">
			<div class="header_in">
				...
				<ul class="menu">
					<li><a href="/research/chulInfo.do" class="btn_menu on">전국절화 농가현황</a></li>
		...
		<div class="content">
			<div class="content_in">
				...
				<div class="search_pack mt70">
					...
					<!-- image button(S) -->
					<a href="javascript:fn_printDiv('printArea');" class="btn_down_image">이미지다운</a>
					<!-- image button(E) -->

				</div>
				<!-- search box pack(E)-->
				
				<div class="img_capture mt30" id="printArea">				
					<!-- 전국절화 농가 현황(S) -->
					<div class="title_box">
						<div class="fl">
							<h4 class="tit_02">전국 절화 농가현황</h4>
						</div>
					</div>
