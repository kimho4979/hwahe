...
<script type="text/javascript">
				...
				if(res.saleDateList!=null && res.saleDateList.length > 0){
					...
					if(val.length > 0) {
						...
						for(var i=0; i<val.length; i++){
							...
							if(i == 0) {
								btnHtml = '<li style="display:flow !important;"><img src="../images/main/chart_btn01.png" alt="차트컬럼이미지1">' + val[i].pumName + ' ' + val[i].qtyRate + '</li>';
							}else if(i == 1) {
								btnHtml = '<li style="display:flow !important;"><img src="../images/main/chart_btn02.png" alt="차트컬럼이미지2">' + val[i].pumName + ' ' + val[i].qtyRate + '</li>';
							}else if(i == 2) {
								btnHtml = '<li style="display:flow !important;"><img src="../images/main/chart_btn03.png" alt="차트컬럼이미지3">' + val[i].pumName + ' ' + val[i].qtyRate + '</li>';
							}else if(i == 3) {
								btnHtml = '<li style="display:flow !important;"><img src="../images/main/chart_btn04.png" alt="차트컬럼이미지4">' + val[i].pumName + ' ' + val[i].qtyRate + '</li>';
							}else if(i == 4) {
								btnHtml = '<li style="display:flow !important;"><img src="../images/main/chart_btn05.png" alt="차트컬럼이미지5">' + val[i].pumName + ' ' + val[i].qtyRate + '</li>';
							}else if(i == 5){
								btnHtml = '<li style="display:flow !important;"><img src="../images/main/chart_btn06.png" alt="차트컬럼이미지6">' + val[i].pumName + ' ' + val[i].qtyRate + '</li>';
							}else {
								btnHtml = '<li style="display:flow !important;"><img src="../images/main/chart_btn07.png" alt="차트컬럼이미지7">' + val[i].pumName + ' ' + val[i].qtyRate + '</li>';
							}
	...
	function getAreaSaleDate(){
		$.ajax({
			type : "post",
			url : "/chart/getAreaSaleDate.json",
			data : {},
			dataType : '',
			success : function(res){
				var saleDateList = res.saleDateList;
...
//							  + '<DataRangeLegend width="200" height="35" minimum="0" maximum="' + max + '" dataRangeHeight="20" fontSize="10" arrowOffset="0" interval="' + mapInteval + '" showArrow="true" tickGap="5" stroke="#000000" arrowColor="#ffffff" arrowStroke="#000000" colors="[#dcdfe8,#93D6F1,#4a88b7,#596283,#5b5e67]"/>'
					...
					$("#searchAreaSaleDate").val(saleDateList[0].saleDate);
	...
	function getAreaChartData(saleDate, areaCode){ 
		...
		$.ajax({
			type : 'get',
			url : "/chart/getAreaChartData.json",
			data : {
				searchSaleDate : saleDate,
				...
				<!-- 상단박스(S) -->
					<div class="c_l_box_top">
						...
						<div class="tit_box">
							...
							<div class="fl">
								<select  aria-label="cmpCdPumMok"  class="select main_g" id="cmpCdPumMok" name="cmpCdPumMok" onchange="getPumMokSaledate();">
									...
									<option value="7368200686">한국화훼(고양)</option>
									<option value="6158209828">영남화훼(김해)</option>
									<%--<option value="1288202296">한국화훼(과천) 영업종료</option>--%>
								</select>
								<label for="cmpCdPumMok"></label> 
								<select aria-label="gubnPumMok" class="select main small" id="gubnPumMok" name="gubnPumMok" onchange="getPumMokSaledate();">
				...
				<div class="conts_center">
					...
					<div class="c_r_box_top">
						...
						<div class="tit_box2 tab">
							...
							<select  aria-label="cmpCdReal"  class="select main" id="cmpCdReal" name="cmpCdReal">
									<option value="0000000001">aT화훼(양재)</option>
									<option value="1508500020">부산화훼(엄궁)</option>
									<option value="6068207466">부경화훼(강동)</option>
									<option value="4108212335">광주원예(풍암)</option>
