...
<script>
	...
	function grid_ajax() {
		...
		$.ajax({
			type : 'get',
			url : "/maa01/maa01MonitorJaeBae.json",
	...
	function grid_ajax1(item) {
		...
		$.ajax({
			type : 'get',
			url : "/maa01/maa01MonitorJaeBaeDetail.json",
	...
	function grid_ajax3(item) {
		...
		$.ajax({
			type : 'get',
			url : "/maa01/maa01MonitorJaeBae_Date.json",
			data : '&COMP_CODE=' + item.COMP_CODE + '&F_DATE=' + F_DATE
					,
			dataType : '',
			success : function(res) {
...
/*           $("#ax-page-btn-search").bind("click", function(){
		...
		modalOpen : function(gridData) {
			...
			myModal.open({
				type : "GET",
				url : "/maa01/Maa01MonitorJaeBaePopup.do",
...
modalOpen1 : function(gridData) {
		...
		getCheckedList : function(index) {
			...
			$.ajax({
				type : 'get',
				url : "/maa01/deleteMonitorJaeBaetDetail.json",
				data : deleteValue,
				dataType : '',
				success : function(res) {
	...
	function chart_ajax() {
		...
		if (F_DATE != '') {
			...
			$.ajax({
				type : 'get',
				url : "/maa01/maa01MonitorJaeBaegDate.json",
				data : '&F_DATE=' + F_DATE ,
				dataType : '',
				success : function(res) {
					console.log(res.list);
				...
				<div class="real_contents">
					...
					<div class="shipment_schedule_all3">
						<div class="shipment_schedule_title">
							...
							<span class="half_title_purple">재배동향 등록 리스트</span>
						</div>
					...
					<div class="shipment_schedule_all3">
						<div class="shipment_schedule_title">
							<div class="half1_purple"></div>
							<span class="half_title_purple">재배동향 리스트</span>
						</div>
						<div class="hw_clear"></div>
						<div class="hw_ma_grid2">
