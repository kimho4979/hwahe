...
<script>
	...
	function grid_ajax() {
		...
		$.ajax({
			type : 'get',
			url : "/maa01/maa01MonitorJakHwang.json",
	...
	function grid_ajax1(item) {
		...
		$.ajax({
			type : 'get',
			url : "/maa01/maa01MonitorJakHwangDetail.json",
	...
	function grid_ajax3(item) {
		...
		$.ajax({
			type : 'get',
			url : "/maa01/maa01MonitorJakHwang_Date.json",
			data : '&COMP_CODE=' + item.COMP_CODE + '&F_DATE=' + F_DATE,
			dataType : '',
			success : function(res) {
				console.log(res.list);
	...
	var fnObj = {
		...
		bindEvent : function() {
			...
			$("#ax-grid-btn-add").bind(
					"click",
					function() {

						if ($('#COMP_CODE').val() != null
								&& $('#COMP_CODE').val() != '') {
							var gridData = "COMP_CODE=" + $('#COMP_CODE').val()
									+ "&SAN_CD=" + $('#SAN_CD').val();
							fnObj.modalOpen1(gridData);
						} else {
							alert('작황정보 등록 리스트를 선택해주세요.');
						}
		...
		modalOpen : function(gridData) {
			...
			myModal.open({
				type : "GET",
				url : "/maa01/Maa01MonitorJakHwangPopup.do",
		...
		getCheckedList : function(index) {
			...
			$.ajax({
				type : 'get',
				url : "/maa01/deleteMonitorJakHwangtDetail.json",
				data : deleteValue,
				dataType : '',
				success : function(res) {
	...
	function chart_ajax() {
		...
		if (F_DATE != '') {
			...
			$.ajax({
				type : 'get',
				url : "/maa01/maa01MonitorJakHwangDate.json",
				data : '&F_DATE=' + F_DATE,
				dataType : '',
				success : function(res) {
					console.log(res.list);
	...
	<div id="wrap">
		...
		<div class="content_wrap">
			...
			<div class="navarea">
				...
				<!-- menu -->
				<input type="hidden" id="stDt" /> <input type="hidden"
					id="MARKET_CD" /> <input type="hidden" id="COMP_CODE" /> <input
					type="hidden" id="SAN_CD" />
				...
				<div class="real_contents">
					...
					<div class="shipment_schedule_all3">
						<div class="shipment_schedule_title">
							...
							<span class="half_title_purple">작황동향 등록 리스트</span>
						</div>
					...
					<div class="shipment_schedule_all3">
						<div class="shipment_schedule_title">
							<div class="half1_purple"></div>
							<span class="half_title_purple">작황동향 리스트</span>
						</div>
