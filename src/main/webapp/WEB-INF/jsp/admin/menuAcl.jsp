...
function grid_ajax(){
	...
	$.ajax({
		type : 'post',
		url : '/sys/menu/auth-config.json',
		data : { 
			id : $.trim($("#searchUserId").val()),
			//loc : $('#searchCPNM1').val(),
...
var fnObj = {
	pageStart: function(){
		...
		/* $.ajax({
			...
			dataType:'json',
			success:function(res){
				$("#AXGridTarget")[0].addJSONData(res.list)
			}
		})
		...
		myGrid.setConfig({
			...
			colGroup : [
				...
				{key:"USER_ID", label:"아이디", width:"120", align:"center",formatter:function(){ if(this.item.USER_GUBUN == 'kakao'){return this.item.USER_ID.replace(this.item.USER_ID.substring(0,this.item.USER_ID.length-4),'*******');}else{return this.item.USER_ID}}},
				{key:"USER_GUBUN", label:"이용자구분", width:"100", align:"center"},
				{key:"USE_YN", label:"사용여부", width:"100", align:"center"},
				{key:"INIT_YN", label:"초기화여부", width:"100", align:"center"},
				//{key:"REG_ID", label:"등록자", width:"120", align:"center",hidden:true},
				{key:"REG_DATE", label:"등록일자", width:"120", align:"center"},
				{key:"ACS_YN", label:"농식품부자료접근", width:"120", align:"center" ,editor:{updateEdit:true,type:"text",updateWith:["_CUD"]},formatter:function(){
					//let out = '<select class="'+this.key+'" style="width:100%;" >';
					//out += '<option data="'+this.item.USER_ID+'" value="'+this.item.ACS_YN+'">'; 
					//out += '<option data="'+this.item.USER_ID+'" value="'+this.item.ACS_YN+'">N</>'; 
					return '<input type="text" class="'+this.key+'"  data="'+this.item.USER_ID+'"   style="width:100%;"  value="'+this.item.ACS_YN+'">'
...
function fn_save(){
	...
	let params = [];
	let paramObj = {};
	$.each(myGrid.getCheckedList(0),function(i,v){
		paramObj = {};
		let userid = '';
		let userid2 = '';
		if(myGrid.getCheckedList(0)[i].USER_ID.indexOf('@') != -1 ){
			//console.log("이메일형식")
			//userid = myGrid.getCheckedList(0)[i].USER_ID.split('@')[0]	
			//userid2 = myGrid.getCheckedList(0)[i].USER_ID.split('@')[1]
			//console.log(userid , userid2)
			//userid = userid + "\\\\@" + userid2;
			//console.log(userid)
			userid = myGrid.getCheckedList(0)[i].USER_ID
		} else{
			userid = myGrid.getCheckedList(0)[i].USER_ID
		}
		paramObj.val = $('[data="'+userid+'"]').val()
		//paramObj.val = $(''+$.escapeSelector(userid)).val()
		paramObj.key = myGrid.getCheckedList(0)[i].USER_ID
...
<div class="wrap">
	...
	<div class="hw_content"><!--컨텐츠 내용들어가는곳 시작-->
		...
		<h3 class="hw_h3_ma"><span class="half_title">사용자 접근권한 설정</span></h3>
		<div class="hw_clear"></div>	
			
		<div class="hw_infouse_01">  <p> <span class="purple"> ◎ 이용안내 :</span> 회원정보에서 농식품부 내부자료 접근가능여부를 설정한다.</p></div>
		...
		<div style="margin-bottom:10px; margin-top:10px;">	
			<div class="selectWrap">
				...
				유저아이디
				<input type="text" id="searchUserId" name="searchUserId" class="AXInput" onkeypress='if(event.keyCode==13) {fn_search(); return false;}'/>
