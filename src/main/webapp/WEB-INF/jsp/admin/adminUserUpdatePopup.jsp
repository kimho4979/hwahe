...
<%   
   if (request.getParameter("regDt")==null){
    ...
    request.setAttribute("oldUserType", request.getParameter("userType"));
    request.setAttribute("compCode", request.getParameter("compCode"));
    request.setAttribute("useYn", request.getParameter("useYn")); 
   ...
   <style type="text/css">
    ...
    var fnObj = {
        pageStart: function(){
        	...
        	//fnObj.upload.init();
        	$("#userId").val("${userId}");
        	$("#oldUserType").val("${userType}");
        	$("#oldCompCode").val("${compCode}");
        	...
        	if (usrType == "M") {
				...
				$("#remarkDiv2").show();
				$("#remarkDiv1").hide();
				$("#compCode1").val("${compCode}").attr("selected", "selected");
				$("#remark").val(remark);
        ...
        form: {			
			submit: function(){
				...
				$.ajax({
			           type : "POST",
			           url : "/admin/saveUpdateUserProc.json",
			           data : $(frm).serialize(),
			           dataType : '',   	           
        ...
        sanCdopen: function(){
            ...
            myModal.open({
                ...
                pars:("&remark="+remark).queryToObject(),
                top:0, width:400,
                closeByEscKey:true
            });
        },
...
<input type="hidden" id="dupCheck" name="dupCheck" value="OK"/>
<input type="hidden" id="oldUserType" name="oldUserType" value=""/>
<input type="hidden" id="oldCompCode" name="oldCompCode" value=""/>
...
<div class="hw_clear"></div>
<input type="text" id="userId" name="userId"  class="AXInput W100" readonly="true"/><!-- <button class="AXButton" type="button" id="ax_btn_001" onclick="fnObj.checkedUserId();">중복체크</button> -->
<div class="hw_clear"></div>	
<h4 class="ma_title">회원구분</h4> <p class="hw_green"></p>
<div class="hw_clear"></div>	
<input type="radio" name="userType" id="userType" value="A" />관리자
<input type="radio" name="userType" id="userType" value="M" />모니터요원
