...
var fnObj = {
    ...
    modalOpen: function (seq, title, insId, insDt, gubn, orFileNm, seqNum) {
    	...
    	if(orFileNm!= ''){
    		var aString = "<a href='/hfa01/hfa01FileDown.json?seq="+seq+"' download><img src='/images/filedownload.gif' alt='파일다운로드' >"+orFileNm+"</a>";
...
function fn_searchPage(pageNo){
   ...
   comSubmit.setUrl("<c:url value='/hba01/hba04.do'/>");
...
<body>
	...
	<h2 class="blind">화훼유통정보시스템 본문</h2>
		<div class="sub_header">
			...
			<h3 class="sub_tit05 mb70">순별장미수급예측보고서</h3><!-- sub_tit end -->
		</div><!-- sub_header end -->
		<div class="contentWrap" id="content">
			<div class="sub_content">
				<section class="section content01">
					...
					<form:form commandName="calculateVo" name="listForm" id="listForm" method="post">
						<table class="boxList">
								...
								<tbody>
								<c:set var="hba03" value="${selectHba01}"/>
								<c:if test="${!empty hba03}">
								<c:forEach var="notice" items="${selectHba01}"  varStatus="status">
									<tr>
										<td>${notice.ANUM}</td>
										<td>
				...
				<ul class="ct_txt mt16">
					...
					<li>순별장미수급예측보고서를 조회 할 수 있는 화면</li>
				</ul><!-- ct_txt end -->
			</div><!-- sub_content end -->
		</div><!-- contentWrap end -->
	</article>
		...
		<div class="lp_container" id="utda_input"><!-- D: ID 필수 -->
			<div class="lp_area">
				...
				<div class="lp_content">
					<h2 class="lp_tit">순별장미수급예측보고서</h2><!-- lp_tit end -->
					<table class="boxList">
						<caption>순별장미수급예측보고서</caption>
						<colgroup>
							<col style="width:30%;">
							<col style="width:*;">
							<col style="width:30%;">
						...
						<tbody>
							...
							<tr>
								<th>내용</th>
								<td colspan="5" id="contentView"></td>
							</tr>
							<tr class="fileUp">
								<th>파일업로드</th>
